...
def plotGeneDepths(file_samtools_depths: str, title: str = "") -> list[go.Figure]:
    ...
    fig_line = px.line(
        df, x="pos", y="depth", color="gene", title=f"Read Depth {title}"
    )
...
def plotReadMappingStat(
    bam_files: list[str], fastq_files: list[str] = [], methods: list[str] | None = None
) -> list[go.Figure]:
    """
