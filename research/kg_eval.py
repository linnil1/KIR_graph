...
def readPredictResult(
    tsv_file: str, extract_func: Callable[[str], str] = extractID
) -> CohortAlleles:
...
def addSequence(
    cohort_results: CohortMatchResult,
    cohort_answer_seqs: CohortSequences,
    cohort_predit_seqs: CohortSequences,
) -> None:
    allele_sequence = SeqIO.to_dict(
        SeqIO.parse("index5/kir_2100_withexon_ab.leftalign.mut01_sequences.fa", "fasta")
    )
    ...
    for id, results in cohort_results.items():
        for result in results:
            if result.answer_allele_full:
                result.answer_seq = cohort_answer_seqs[id][
                    result.answer_allele_full
                ].seq
            ...
            if id in cohort_predit_seqs and result.predit_allele_full:
                result.predit_seq = cohort_predit_seqs[id][
                    result.predit_allele_full
                ].seq
            else:
...
def compareSample(answer_list: list[str], predict_list: list[str]) -> list[MatchResult]:
    ...
    for gene in answer_dict.keys() | predit_dict.keys():
        ...
        #     continue
        gene_comparison_result.extend(compareGene(answer_dict[gene], predit_dict[gene]))
    ...
    for allele_b in list(b_list):
        for allele_a in a_list:
            if getAlleleField(allele_a, 7) == getAlleleField(allele_b, 7):
                ...
                yield MatchResult(
                    limitAlleleField(allele_a, 7),  # allele name (remove novel suffix)
...
def getBaseMatchness(result: MatchResult) -> dict[str, int]:
    ...
    if result.match_type in [
        ...
        score = pairwise2.align.localxx(
            result.answer_seq, result.predit_seq, score_only=True
        )
...
def addBaseMatchness(results: CohortMatchResult) -> None:
    ...
    with ProcessPoolExecutor() as executor:
        base_results = executor.map(
            getBaseMatchness, chain.from_iterable(results.values())
        )
        for result, base_result in zip(
            chain.from_iterable(results.values()), base_results
        ):
            if not base_result:
                continue
...
def printSummaryByResolution(cohort_results: CohortMatchResult) -> dict[str, int]:
    ...
    Return:
      summary: The value of each metrics
    """
    summary = calcSummaryByResolution(
        result for results in cohort_results.values() for result in results
    )
...
def plotGeneLevelSummary(
    ...
    df_plot = pd.melt(
        df,
        id_vars="gene",  # type: ignore
        value_vars=["7digits_acc", "5digits_acc", "3digits_acc", "gene_acc"],
        value_name="accuracy",
        var_name="level",
    )
...
def printSummaryGeneLevel(
    cohort_results: CohortMatchResult, print_text: bool = True
) -> pd.DataFrame:
    """
...
def compareAlleleWithMethod(cohort_data: dict[str, CohortAlleles]) -> None:
    ...
    for method, dat in cohort_data.items():
        cohort_results[method] = compareCohort(
            cohort_data["answer"], dat, skip_empty=False, verbose_sample=False
        )
    ...
    with pd.option_context("display.max_rows", None, "display.max_columns", None, "display.width", 180):  # type: ignore
        printDf(summary.groupby(["gene", "method"]).sum())
