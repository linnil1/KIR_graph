# 06_genotype_call_with_ploidy.py
import gzip
import numpy as np
import sys
import pandas as pd
...
difpos_file = sys.argv[2]  # "./data/"+gene+".difpos.all.txt"
dosage_file = sys.argv[3]
reference_file = sys.argv[4]
output_allele_file = sys.argv[5]
gene = sys.argv[6]
sample = sys.argv[7]
...
gene_var = defaultdict(list)
var_dosage = {}
gene_TypeDetPos = defaultdict(list)
var_TypeDetDose = {}
gene_header = {}
ploidy_gene = {}

with open(difpos_file,"rt") as file:
    for line in file:
        if line.find("#") == -1:
            line = line.rstrip().split("\t")
            gene_TypeDetPos[gene].append([line[0],line[1],line[2]])
            ...
            if line[5] != "LowQual":
                ...
                for d in dose:
                    if d != ".":
                        ...
                        break
                if dosage != "NA":
                    g = {'KIR2DL5A;KIR2DL5B': "KIR2DL5AB", "KIR2DS3;KIR2DS5": "KIR2DS35"}.get(line[0], line[0])
                    ploidy_gene[g] = len(dose)
                    var_dosage[tuple([g,line[1],line[3],line[4]])] = dosage
                    gene_var[g].append([line[1],line[3],line[4]])

if gene not in ploidy_gene:
    exit()
ploidy = ploidy_gene[gene]
print(sample, gene, "ploidy", ploidy)
